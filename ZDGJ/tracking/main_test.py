...
flags.DEFINE_float('GPU_MEMORY', 0.0, 'GPU_MEMORY')
...
#                    break
    endtime = time.time()
#    f = open("/data/ZDGJ/tracking/log_%s.txt"%(str(time.strftime('%Y-%m-%d_%H.%M.%S',time.localtime(time.time())))),'a')
    f = open("/data/ZDGJ/tracking/log_%s.txt"%(str(time.strftime('%Y-%m-%d',time.localtime(time.time())))),'a')
    f.write(str(time.strftime('%Y-%m-%d_%H.%M.%S',time.localtime(time.time())))+'\n')
    f.write(str(FLAGS.CAMERA_IP)+' needs time: '+ str(endtime-starttime)+'\n')
    f.write('GPU_MEMORY: '+str(FLAGS.GPU_MEMORY)+'\nPATH_TO_CKPT: '+str(FLAGS.PATH_TO_CKPT)+'\n\n')
    f.close()
    print(str(FLAGS.CAMERA_IP)+' needs time: '+ str(endtime-starttime))
